...
import edu.ucsd.sbrg.fixing.SBMLFixer;
...
import edu.ucsd.sbrg.parameters.ParametersException;
...
import edu.ucsd.sbrg.reporting.ProgressFinalization;
...
public class ModelPolisherCLILauncher extends Launcher {
  ...
  public void commandLineMode(AppConf appConf) {
    ...
    if (args.containsKey(ConfigOptions.CONFIG)) {
      try {
        parameters = new ParametersParser().parseCLIParameters(new File(args.getProperty(ConfigOptions.CONFIG)));
      } catch (IOException e) {
      ...
      if (parameters.input().isDirectory()) {
        ...
        inputOutputPairs.parallelStream().forEach(pair -> {
          try {
            ...
            logger.info(format("Skipping unreadable file \"{0}\".", pair.getLeft()));
